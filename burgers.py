...
# from vis import plot_and_save_data
from utils import save_expert_gates
...
if __name__ == '__main__':
    ...
    data = scipy.io.loadmat('/root/burgers_shock.mat')
    ...
    for ms in model_seeds:
        ...
        gates_grid = np.zeros((T.shape[0], T.shape[1], num_experts))
        for i in range(num_experts):
            gates_grid[:, :, i] = griddata(X_star, gates_pred[:, i], (X, T), method='cubic')
        gates_save_dir = f"burgers_gates_{ms}"
        os.makedirs(gates_save_dir, exist_ok=True)
        save_expert_gates(T, X, gates_grid, gates_save_dir, ms)
        print(f"Saved expert gate plots to {gates_save_dir}")