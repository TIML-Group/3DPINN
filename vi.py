...
def vi_wave(model, res: int = 256, outdir: str = "expert_vis", c: int = 2, ep: int = 9999):
    ...
    # ft_vals = _unit(ft_vals)
    # fx_vals = _unit(fx_vals)
    # target_t = _unit(target_t)
    ...
    #         if name == 'ft':
    #             if np.dot(arr[:, j], target_t) < 0:
    #                 arr[:, j] *= -1
    #             plt.plot(axis_vals, arr[:, j], label=f"pred ft")
    #             plt.plot(axis_vals, target_t, label=f"true ft")
    #         if name == 'fx':
    #             plt.plot(axis_vals, arr[:, j], label=f"pred fx")
    #             plt.plot(axis_vals, target_x, label=f"true fx")

    #     plt.xlabel("t" if name == "ft" else "x")
    #     plt.ylabel(f"{name}(t)" if name == "ft" else f"{name}(x)")
    #     plt.title(f"{name}-step{ep}")
    #     plt.legend()
    #     plt.tight_layout()

    #     fname = os.path.join(outdir, f"{name}-step{ep}.png")
