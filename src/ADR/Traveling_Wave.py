...
# def T_time(t: torch.Tensor) -> torch.Tensor:
#     """Temporal mode T(t) = exp(-gamma t) cos(omega t)."""
#     return torch.exp(-GAMMA * t) * torch.cos(OMEGA * t)
...
def u_exact(coords: torch.Tensor) -> torch.Tensor:
    ...
    z = (x - X0 - C_FRONT * t) / EPS
    return m - a * torch.tanh(z)
...
def source_S(t: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
    ...
    # derivatives of tanh front
    # u_t = a*(C_FRONT/EPS)*sech^2(z)
    u_t = a * (C_FRONT / EPS) * sech2
    # u_x = -a*(1/EPS)*sech^2(z)
    u_x = -a * (1.0 / EPS) * sech2
    # u_xx = 2a*(1/EPS^2)*sech^2(z)*tanh(z)
    u_xx = 2.0 * a * (1.0 / (EPS**2)) * sech2 * th

    lam = lambda_x(x)          # keep your original lambda(x)
    ueq = u_eq_x(x)            # keep your original u_eq(x)
...
class PINN_AdvecReactDiff:
    ...
    @torch.no_grad()
    def visualize(self, ep=0, out_dir="ADR", Nt=200, Nx=400, device="cuda:4"):

        os.makedirs(out_dir, exist_ok=True)
        t_vals, x_vals, coords = ADR_slice_sampler()
        u_pred, gates = infer_on_coords(self.pinn, coords, normalize_fn=self._normalize, device=device)
        ...
        png = os.path.join(out_dir, f"ADR_slice_ep{ep:05d}.png")
    ...
    def train(
        ...
        for ep in range(1, epochs_adam + 1):
            ...
            if ep % 2000 == 0:
            #     self.visualize(ep=ep, out_dir="ADR")
                vi = vi_ADR(model=self.pinn, res=256)
...
if __name__ == "__main__":
    ...
    # for expert_rank in r_values:
        ...
        model = PINN_AdvecReactDiff(pinn, global_test_coords)
        ...
        print(f"Final L2 Relative Error: {final_error:.4e}")
        final_vi = vi_ADR(model=model.pinn, res = 256)

        result_key = f"ADR_seed_{ms}"
        ...
        file_path = f"training_results_ADR(TW)_r={expert_rank}.json"
